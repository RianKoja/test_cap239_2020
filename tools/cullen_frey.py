...
def cullenfrey_chart(xd, yd, legend, title):
    ...
    ax.plot(np.arange(0, poly_x1, step), 3/2. * np.arange(0, poly_x1, step) + 3, label="gamma", linestyle='-', c='black')
    ax.plot(np.arange(0, poly_x1, step), 2 * np.arange(0, poly_x1, step) + 3, label="lognormal", linestyle='-.', c='black')
    ax.legend()
    ax.set_ylim(y_lim, 0)
...
def series2chart(series):
    skewness_square = (moment(series, moment=3)) ** 2
    kurtosis = moment(series, moment=4) + 3
    print("skewness_square =", skewness_square, " and kurtosis = ", kurtosis)
    cullenfrey_chart([skewness_square], [kurtosis], "Data for USA", "Daily new confirmed COVID-19 cases")
